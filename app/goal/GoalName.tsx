...
const GoalName = () => {
  ...
  const resetCurrentGoal = useAppStore((state) => state.resetCurrentGoal);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.actionText}>
          {!dueDate && !showDatePicker ? "Goal name" : "Goal date"}
        </Text>
        <TouchableOpacity onPress={onNext}>
          <Text
            style={{
              ...styles.actionText,
              color: isFormComplete ? "#7489DF" : "#5698ee",
            }}
          >
            {" "}
            {!isFormComplete ? "Next" : "Done"}
          </Text>
        </TouchableOpacity>
      </View>
      <View style={styles.imageContainer}>
        <ImageBackground source={{ uri: currentGoal.uri }} style={styles.image}>
          <LinearGradient
